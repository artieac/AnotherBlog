...
namespace AlwaysMoveForward.AnotherBlog.Web.Controllers.API
{
    public class BlogPostController : ApiControllerBase
    {
        ...
        {
            return this.Services.BlogEntryService.GetById(id);
        }

        // GET api/<controller>/1999/1
        public IList<BlogPost> Get(int year, int month)
        {
            DateTime targetDate = new DateTime(year, month, 1);
            return this.Services.BlogEntryService.GetByMonth(targetDate);
        }
        ...
        public IList<BlogPost> Get(string blogSubfolder, int year, int month, int day, string title)
        {
            DateTime targetDate = new DateTime(year, month, day);
            Blog targetBlog = this.Services.BlogService.GetBySubFolder(blogSubfolder);
